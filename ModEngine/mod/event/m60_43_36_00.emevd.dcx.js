...
$Event(0, Default, function() {
    ...
    SetEventFlagID(1043369200, ON); // NEW
    SetEventFlagID(1043369201, ON); // NEW
    InitializeEvent(0, 1043360801, 0); // NEW
    //SetEventFlagID(1043360800, OFF); // NEW
});
...
$Event(1043360801, Restart, function() { // NEW
    if (EventFlag(1043360800)) {
        DisableCharacter(1043360800);
        DisableCharacterCollision(1043360800);
        ForceCharacterDeath(1043360800, false);
        EndEvent();
    }
L0:
    WaitFor(EntityInRadiusOfEntity(10000, 1043360800, 35, 1));
L2:
    SetNetworkUpdateRate(1043360800, true, CharacterUpdateFrequency.AlwaysUpdate);
    DisplayBossHealthBar(Enabled, 1043360800, 0, 904500600);
});
